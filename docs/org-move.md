...
| Is the package published to a FHIR package registry? | **No.** Modules consume the template as the vendored `ig-template/` folder (`ig.ini`: `template = #ig-template`), re-vendored from the companion repository's `dev` branch by `scripts/sync-ig-template.sh`. Publication is the explicit decision [#2](../../../issues/2); the switch a module then performs is [docs/recipes/switch-template-to-published.md](recipes/switch-template-to-published.md). |
...
| Who owns the template after 2026? | **The MII**, for now. MII funding ends end-2026 and **NUM-DIZ takes over IG development and maintenance**; the formal ownership decision stays with the TF KDS: [#7](../../../issues/7). |
