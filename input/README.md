...
| `data/` | **Optional** Jekyll data files the IG template reads as `site.data.<file>.<key>`. `features.json` carries the settings of the template-contributed *Validate an instance* page: `validator.url` (the validator the page's live box posts to; default `https://validator.fhir.org`) and `validator.tx` (the terminology server the page prints in its command lines and passes to the validator; default `https://tx.fhir.org`) — a DIZ points both at its self-hosted validator wrapper and Ontoserver, see `docs/recipes/validate-instances.md`. The same file may carry the base template's `feedback` dashboards. `brand.json` (`design`) selects the header logo variant (documented in the IG template). Every key has a default: a missing file or key changes nothing. | IG Publisher (the base template's pre-process copies the folder to Jekyll's `_data`) |
